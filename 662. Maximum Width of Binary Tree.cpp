 ...
 * Reference: http://www.cnblogs.com/grandyang/p/7538821.html
 *
 * Author: Adam Yu Wen
 * Date: April 11, 2018
 */
...
public:
	int widthOfBinaryTree(TreeNode* root) {
		vector<int> start;
		int res = 0;
		helper(root, 1, 0, start, res);
		return res;
	}

	void helper(TreeNode* root, int h, int idx, vector<int> &start, int &res) {
		if (!root) return;
		if (h > start.size()) start.push_back(idx);
		res = max(res, idx - start[h - 1] + 1);
		helper(root->left, h + 1, idx * 2, start, res);
		helper(root->right, h + 1, idx * 2 + 1, start, res);
	}
};