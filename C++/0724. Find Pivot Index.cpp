...
public:
	int pivotIndex(vector<int>& nums) {
		...
		for (int idx = 0; idx < numsSize; idx++) {
			if (leftSum == rightSum - nums[idx]) return idx;
...
// Method 2:
// Using multiplication instead of subtraction will make the
// program faster. It takes 44 ms.
class Solution {
public:
	int pivotIndex(vector<int>& nums) {
		int leftSum = 0, rightSum = 0;
		...
		for (int idx = 0; idx < numsSize; idx++) {
			if (leftSum * 2 == rightSum - nums[idx]) return idx;
			leftSum = leftSum + nums[idx];
		}
		return -1;
	}
