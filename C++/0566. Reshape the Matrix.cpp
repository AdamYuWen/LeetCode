...
public:
    vector<vector<int>> matrixReshape(vector<vector<int>>& nums, int r, int c) {        
        int numRow = nums.size();
        // If the nums is empty
        if (!numRow) {
            return nums;
        }
        int numCol = nums[0].size();
        // If the 'reshape' operation with given parameters is NOT possible
        if (numRow * numCol != r * c) {
            return nums;
        }
        // Else
        ...
        for (int i = 0; i < numRow; i++) {
            for (int j = 0; j < numCol; j++) {
                res[(i * numCol + j) / c][(i * numCol + j) % c] = nums[i][j];
            }
        }
        return res;
    }
